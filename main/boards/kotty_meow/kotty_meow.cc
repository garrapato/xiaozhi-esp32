...
class KottyMeow : public WifiBoard {
private:
    ...
    {
        auto* self = static_cast<KottyMeow*>(arg);
...
#else
#error "Kotty Meow board requires i2c_bus_get_internal_bus_handle() support"
#endif
        if (!i2c_bus_) {
    ...
    static void touch_event_task(void* arg)
    {
        ...
        while (true) {
            if (touchpad->WaitForTouchEvent()) {
                ...
                auto &board = (KottyMeow &)Board::GetInstance();
    ...
    void InitializeCst816sTouchPad()
    {
        ...
        gpio_isr_handler_add(TP_PIN_NUM_INT, KottyMeow::touch_isr_callback, cst816s_);
    }
    ...
    {
        (void)handle;
        auto* self = static_cast<KottyMeow*>(cb_arg);
...
public:
    ~KottyMeow() {
        ...
        // Since KottyMeow is a singleton that lives for the device lifetime, this is acceptable.
    ...
    KottyMeow() : boot_button_(BOOT_BUTTON_GPIO)
    {
        const esp_timer_create_args_t emotion_timer_args = {
            .callback = &KottyMeow::emotion_reset_timer_callback,
            .arg = this,
